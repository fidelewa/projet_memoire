...
BEGIN
	...
	FROM tab 
	WHERE tname = 'MOYENNE_SEM1_IDA2A';

    -- Si la table existe
    IF(v_nCount=1) THEN

        -- Purge
        v_sql:='DELETE FROM MOYENNE_SEM1_IDA2A';
        EXECUTE IMMEDIATE v_sql;

        -- INSERTION
        EXECUTE IMMEDIATE 'INSERT INTO MOYENNE_SEM1_IDA2A
        ...
        HAVING ((moyenne_session.classe_nom=''IDA2/A'') AND (moyenne_session.session_nom=''1er Semestre''))';
    ...
    IF(v_nCount=0) THEN -- Si la table n'existe pas
        ...
        v_sql := 'CREATE TABLE MOYENNE_SEM1_IDA2A(
