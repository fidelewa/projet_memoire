...
DECLARE
	...
	--TYPE tableMoyenne_tytab IS TABLE OF user_tables.table_name%TYPE INDEX BY BINARY_INTEGER;
	--tab_tableMoyenne tableMoyenne_tytab;
BEGIN
	...
	LOOP
		...
		-- cela signifie qu'aucune de ces tables n'existent en BDD et qu'ils faut les crééer
		IF(PAQUET_CURSEUR.moyenne_sem1_ida2a_cur%NOTFOUND AND PAQUET_CURSEUR.moyenne_sem1_ida2a_cur%ROWCOUNT=0) THEN
		...
		ELSE -- sinon si le dernier fetch (extraction) a renvoyé des lignes, cela signifie que les tables des moyennes
		...
		-- On sortira de la boucle si effectivement aucune ligne n'a été extraite du curseur (curseur vide)
		...
		EXIT WHEN PAQUET_CURSEUR.moyenne_sem1_ida2a_cur%NOTFOUND OR PAQUET_CURSEUR.moyenne_sem1_ida2a_cur%NOTFOUND IS NULL;
