...
header_releve.affecte, body_releve_sem2_ida2a.discipline, body_releve_sem2_ida2a.valeur_1, body_releve_sem2_ida2a.valeur_2, 
body_releve_sem2_ida2a.valeur_3, body_releve_sem2_ida2a.valeur_4, body_releve_sem2_ida2a.valeur_5, 
body_releve_sem2_ida2a.moyenne_matiere, body_releve_sem2_ida2a.coefficient, body_releve_sem2_ida2a.moyenne_ponderee, 
body_releve_sem2_ida2a.appreciation_matiere, body_releve_sem2_ida2a.rang_matiere, body_releve_sem2_ida2a.professeur, 
rqt_rang_sem2_ida2a.rang_classe_ida2a, rqt_points_generaux_sem2.moyenne_min_classe, rqt_points_generaux_sem2.moyenne_max_classe, 
    ...
    (
        ...
        (
            ...
            (
                ...
                FROM moyenne_sem2_ida2a
                ORDER BY moyenne_sem2_ida2a.moyenne_session DESC
            ) rqt_rang_sem2_ida2a ON footer_releve_sem2.releve_numero = rqt_rang_sem2_ida2a.releve_numero
        ) ON rqt_points_generaux_sem2.classe_nom = rqt_rang_sem2_ida2a.classe_nom
    ) INNER JOIN 
    (
                            ...
                            (
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_analyse_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_anglais_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_assidute_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_compta_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_conduite_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_economie_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_francais_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_math_fin_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_math_gene_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_nego_info_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_projet_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_teleinfo_sem2_ida2a
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
...
) body_releve_sem2_ida2a ON header_releve.releve_numero = body_releve_sem2_ida2a.releve_numero

ORDER BY footer_releve_sem2."Moyenne semestrielle" DESC , header_releve.releve_numero, body_releve_sem2_ida2a.discipline;