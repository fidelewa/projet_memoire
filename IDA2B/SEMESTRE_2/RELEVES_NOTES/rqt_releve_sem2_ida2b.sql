...
header_releve.affecte, body_releve_sem2_ida2b.discipline, body_releve_sem2_ida2b.valeur_1, body_releve_sem2_ida2b.valeur_2, 
body_releve_sem2_ida2b.valeur_3, body_releve_sem2_ida2b.valeur_4, body_releve_sem2_ida2b.valeur_5, 
body_releve_sem2_ida2b.moyenne_matiere, body_releve_sem2_ida2b.coefficient, body_releve_sem2_ida2b.moyenne_ponderee, 
body_releve_sem2_ida2b.appreciation_matiere, body_releve_sem2_ida2b.rang_matiere, body_releve_sem2_ida2b.professeur, 
rqt_rang_sem2_ida2b.rang_classe_ida2b, rqt_points_generaux_sem2.moyenne_min_classe, rqt_points_generaux_sem2.moyenne_max_classe, 
rqt_points_generaux_sem2.moyenne_gene_classe, footer_releve_sem2.*
...
(
    (
        (
            ...
            FROM total_point
            WHERE session_nom='2eme Semestre'
            GROUP BY classe_nom
            HAVING classe_nom='IDA2/B'

        ) rqt_points_generaux_sem2 INNER JOIN 
        (
            (
                ...
                (
                    (
                        (
                            SELECT effectif_total_par_classe.classe, nb_eleve_sup_12_sem2_classe."Elèves au-dessus de 12", 
                            nb_eleve_10_12_sem2_classe."Elèves entre 10 et 12", 
                            nb_eleve_09_10_sem2_classe."Elèves entre 09 et 10", 
                            nb_eleve_inf_09_sem2_classe."Elèves en dessous de 09", 
                                    ...
                                    (
                                        (
                                            (
                                                ...
                                                HAVING session_nom='2eme Semestre'
                                            ) nb_eleve_10_12_sem2_classe LEFT JOIN 
                                            (
                                                ...
                                                GROUP BY classe_nom, session_nom
                                                HAVING session_nom='2eme Semestre'
                                            ) nb_eleve_sup_12_sem2_classe ON nb_eleve_10_12_sem2_classe.classe_nom = nb_eleve_sup_12_sem2_classe.classe_nom
                                        ) LEFT JOIN 
                                        (
                                            ...
                                            GROUP BY classe_nom, session_nom
                                            HAVING session_nom='2eme Semestre'
                                        ) nb_eleve_09_10_sem2_classe ON nb_eleve_10_12_sem2_classe.classe_nom = nb_eleve_09_10_sem2_classe.classe_nom
                                    ) LEFT JOIN 
                                    (
                                        ...
                                        GROUP BY classe_nom, session_nom
                                        HAVING session_nom='2eme Semestre'
                                    ) nb_eleve_inf_09_sem2_classe ON nb_eleve_10_12_sem2_classe.classe_nom = nb_eleve_inf_09_sem2_classe.classe_nom
                                ) LEFT JOIN 
                                (
                                    ...
                                    GROUP BY classe_nom
                                ) nb_eleve_non_classe ON nb_eleve_10_12_sem2_classe.classe_nom = nb_eleve_non_classe.classe_nom
                            ) ON effectif_total_par_classe.classe = nb_eleve_10_12_sem2_classe.classe_nom
                        ) repartition_eleve_moy_sem2 INNER JOIN
                        (
                            classe INNER JOIN
                            (
                        ...
                        ) ON repartition_eleve_moy_sem2.classe = classe.nom
            ...
            (
                ...
                CASE WHEN RANK() OVER (ORDER BY MOYENNE_SESSION DESC)=1 
                    ...
                    END AS rang_classe_ida2b
                FROM moyenne_sem2_ida2b
                ORDER BY moyenne_sem2_ida2b.moyenne_session DESC
            ) rqt_rang_sem2_ida2b ON footer_releve_sem2.releve_numero = rqt_rang_sem2_ida2b.releve_numero
        ) ON rqt_points_generaux_sem2.classe_nom = rqt_rang_sem2_ida2b.classe_nom
    ) INNER JOIN 
    (
        ...
        CASE 
            ...
            ELSE ''
        END AS eleve_sexe, sess.nom AS session_nom, etablissement.nom AS etab_nom, etablissement.code AS etab_code,
    ...
    ) header_releve ON footer_releve_sem2.releve_numero = header_releve.releve_numero
) INNER JOIN 
(
    SELECT classement_ida2b.releve_numero, libelle AS discipline, 
        ...
        (professeur INNER JOIN 
            (
                            ...
                            (
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_analyse_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_anglais_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_assidute_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_compta_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_conduite_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_economie_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_francais_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_math_fin_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_math_gene_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_nego_info_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_projet_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_teleinfo_sem2_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_vb_sem2_ida2b
                                
                            )
                            classement_ida2b INNER JOIN 
                            (
                                ...
                                FROM note
                            ) note ON (classement_ida2b.releve_numero = note.releve_id) AND (classement_ida2b.matiere_code = note.matiere_code)
                ...
                (
                    ...
                    ON matiere.code = matiere_classe.matiere_code) 
                    INNER JOIN PROFESSEUR_MATIERE ON matiere.code = PROFESSEUR_MATIERE.matiere_code) ON professeur.id = PROFESSEUR_MATIERE.professeur_id
                    WHERE ((professeur.nom='ADOPO') And (professeur.prenom='Frederique') And (classe.nom='IDA2/A')) 
                )
    ...
    WHERE (classe.nom='IDA2/B')
    ORDER BY classement_ida2b.releve_numero, matiere.code

) body_releve_sem2_ida2b ON header_releve.releve_numero = body_releve_sem2_ida2b.releve_numero

ORDER BY footer_releve_sem2."Moyenne semestrielle" DESC , header_releve.releve_numero, body_releve_sem2_ida2b.discipline;