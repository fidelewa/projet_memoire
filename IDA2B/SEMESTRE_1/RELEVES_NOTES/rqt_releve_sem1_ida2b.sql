...
header_releve.affecte, body_releve_sem1_ida2b.discipline, body_releve_sem1_ida2b.valeur_1, body_releve_sem1_ida2b.valeur_2, 
body_releve_sem1_ida2b.valeur_3, body_releve_sem1_ida2b.valeur_4, body_releve_sem1_ida2b.valeur_5, 
body_releve_sem1_ida2b.moyenne_matiere, body_releve_sem1_ida2b.coefficient, body_releve_sem1_ida2b.moyenne_ponderee, 
body_releve_sem1_ida2b.appreciation_matiere, body_releve_sem1_ida2b.rang_matiere, body_releve_sem1_ida2b.professeur, 
rqt_rang_sem1_ida2b.rang_classe_ida2b, rqt_points_generaux_sem1.moyenne_min_classe, rqt_points_generaux_sem1.moyenne_max_classe, 
    ...
    (
        ...
        (
            ...
            (
                ...
                FROM moyenne_sem1_ida2b
                ORDER BY moyenne_sem1_ida2b.moyenne_session DESC
            ) rqt_rang_sem1_ida2b ON footer_releve_sem1.releve_numero = rqt_rang_sem1_ida2b.releve_numero
        ) ON rqt_points_generaux_sem1.classe_nom = rqt_rang_sem1_ida2b.classe_nom
    ) INNER JOIN 
    (
                            ...
                            (
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_analyse_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_anglais_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_assidute_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_compta_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_conduite_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_economie_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_francais_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_math_fin_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_math_gene_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_nego_info_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_projet_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
                                ...
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        ...
                                        ELSE RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'ème'
                                    END AS rang_matiere, matiere_code, releve_numero
                                FROM moyenne_teleinfo_sem1_ida2b
                                UNION
                                SELECT 
                                    CASE WHEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)=1 
                                        THEN RANK() OVER (ORDER BY MOYENNE_MATIERE DESC)||' '||'er'
...
) body_releve_sem1_ida2b ON header_releve.releve_numero = body_releve_sem1_ida2b.releve_numero

ORDER BY footer_releve_sem1."Moyenne semestrielle" DESC , header_releve.releve_numero, body_releve_sem1_ida2b.discipline;